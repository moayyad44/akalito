...
// ══════════════════════════════════════════════════════════
// نظام الإشعارات — مشترك بكل صفحات لوحة الأدمن (البادج بالسايدبار وشاشة الإشعارات)
// `admin_notifications`: تنبيهات تدخل تلقائياً للأدمن (طلب جديد، طلب جاهز...الخ)
// `user_notifications`: رسائل يبعتها الأدمن للزبائن/السائقين
// ══════════════════════════════════════════════════════════

/* يراقب عدد التنبيهات غير المقروءة بالوقت الحقيقي وينادي callback(count) عند أي تغيير.
   يرجّع دالة unsubscribe (استدعيها لو الصفحة بتتفكك يدوياً، عادة مش ضروري بصفحات الأدمن). */
export function watchUnreadNotifications(callback) {
